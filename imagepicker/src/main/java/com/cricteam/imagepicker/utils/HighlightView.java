...
class HighlightView {
    ...
    private void init() {
        ...
        mResizeDrawableWidth =
                resources.getDrawable(R.drawable.camera_crop_width); // TODO :  This method was deprecated in API level 22. Use getDrawable(int, Theme) instead. 
        ...
        if (!hasFocus()) {
            ...
            if (mMode == ModifyMode.Grow) {
                if (mCircle) {
                    ...
                    int d = (int) Math.round(StrictMath.cos(/*45deg*/Math.PI / 4.0D)
                            * (mDrawRect.width() / 2.0D));
    ...
    private void growBy(float dx, float dy) {
        ...
        if (dx > 0.0F && r.width() + 2 * dx > mImageRect.width()) {
            dx = (mImageRect.width() - r.width()) / 2.0F;
            if (mMaintainAspectRatio) {
                dy = dx / mInitialAspectRatio;
            }
        }
        if (dy > 0.0F && r.height() + 2 * dy > mImageRect.height()) {
            dy = (mImageRect.height() - r.height()) / 2.0F;
            if (mMaintainAspectRatio) {
                dx = dy * mInitialAspectRatio;
            }
        }
        ...
        if (r.left < mImageRect.left) {
            r.offset(mImageRect.left - r.left, 0.0F);
        } else if (r.right > mImageRect.right) {
            r.offset(-(r.right - mImageRect.right), 0);
        }
        if (r.top < mImageRect.top) {
            r.offset(0.0F, mImageRect.top - r.top);
        } else if (r.bottom > mImageRect.bottom) {
            r.offset(0.0F, -(r.bottom - mImageRect.bottom));
        }
